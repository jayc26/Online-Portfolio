...
$sqlh = "SELECT * FROM jobprofile where jid='$eh'";
...
if($_SERVER["REQUEST_METHOD"] == "POST")
{
    ...
    if(empty(trim($_POST['jp'])))
      {
        $jp_err = "Please Enter Valid First Name";
      } 
      else
      {
      ...
      {
        $r_err = "Please Enter Valid Last Name";
      } 
      else
      {
      ...
      if(empty($jp_err) && empty($r_err))
      {
        $qe="update jobprofile set job_profile='$jp', rate='$r' where jid='$eh'";
        mysqli_query($conn, $qe);
        $q="insert into logs(name,page,action) values('$jp','Hire','Edited')";
            mysqli_query($conn, $q);
      }
      $at=$_POST['at'];
      $i=1;
      foreach ($at as $key => $atr) {
        ...
        $qr="update perks set project='$jp', pd='$atr' where project='$pn' and pid='$i'";
...
<?php }?>
        ...
        <section style="align-content: center;align-self:center; margin-left: 20px">
            ...
            <?php 
            $sql = "SELECT * FROM jobprofile where jid='$eh'";
            ...
            <input type="text" style="float: left; margin-left: 40px" name="jp" value="<?php echo $row['job_profile']?>" required/>
            <div>
            ...
            <input type="number" style="float: left; margin-left: 40px" name="r" value="<?php echo $row['rate']?>" required/>
            <div>
            ...
            <input type="text" style="float: left;margin-left: 50px" name="at[]" value="<?php echo $row1['pd']?>" required/>
            <div>
